...
- (nullable instancetype) initWithTGAFileAtLocation:(NSURL *)location
{
    self = [super init];
    if(self)
    {
        ...
        typedef struct __attribute__ ((packed)) TGAHeader
        {
            ...
            //About union keyword: https://www.binpress.com/objective-c-structs-unions/
            union
            {
                struct
                {
        ...
        // Copy the entire file to this fileData variale
        ...
        /*
         ...
         are converted to a 32-bit BGRA format that Metal does understand(MTLPixelFormatBGRA8Unorm)
         */
        ...
        /*
         [TGA Spec]
         Real Image data is immmediatly after the header and the ID.
         So set the pointer to (file's start + size of the header + size of the ID)
        ...
        for(NSUInteger y = 0; y < _height; y++)
        {
            ...
            for(NSUInteger x = 0; x < _width; x++)
            {
                ...
                NSUInteger dstPixelIndex = 4 * (y * _width + x);        //4: BGRA
